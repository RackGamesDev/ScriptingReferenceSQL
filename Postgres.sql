...
DROP TABLE IF EXISTS USUARIO_TELEFONO CASCADE;
DROP TABLE IF EXISTS CASA CASCADE;
DROP TABLE IF EXISTS VEHICULO CASCADE;

--Crear tabla con esas propiedades
CREATE TABLE USUARIO(uuid SERIAL PRIMARY KEY, nombre VARCHAR(127) NOT NULL, apellido VARCHAR(255), fecha DATE DEFAULT '2020-12-31', numero NUMERIC(5, 2), calificacion INT DEFAULT 1, activo BOOLEAN DEFAULT TRUE, creado TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE TELEFONO(id INT, prefijo VARCHAR(5), numero VARCHAR(9), fecha DATE, activo INT DEFAULT 0 CHECK (activo IN (0, 1)));
CREATE TABLE USUARIO_TELEFONO(uuid_USUARIO SERIAL, id_TELEFONO INT); --Esta tabla sale de una relacion n:m
CREATE TABLE CASA(id SERIAL PRIMARY KEY, uuid_USUARIO SERIAL); --Esta tabla tendra una clave ajena a la pk de otra tabla
CREATE TABLE VEHICULO(id SERIAL PRIMARY KEY, nombre VARCHAR(9), uuid_USUARIO SERIAL, CONSTRAINT fk_VEHICULO_uuid_USUARIO FOREIGN KEY (uuid_USUARIO) REFERENCES USUARIO(uuid)); --Se pueden hacer las restricciones directamente en la tabla, pero no se recomienda
...
INSERT INTO TELEFONO (id , numero, fecha) VALUES (0, '123456789', TO_DATE('21/03/2022', 'DD/MM/YYYY')); --Aagnadir un entry a la tabla, TO_DATE se puede reemplazar por CURRENT_TIMESTAMP para poner la fecha actual
INSERT INTO TELEFONO VALUES (1, '+52', '123456789', TO_DATE('21/03/2022', 'DD/MM/YYYY'), 0); --Si se agnaden todos los campos no hace falta ponerlos
INSERT INTO TELEFONO VALUES (2, '+34', '123412344'), (3, '+34', '123412344'), (4, '+34', '123412344'); --Insertar muchos registros a la vez
...
TRUNCATE TABLE TELEFONO CASCADE; --Borra todos los entrys de una tabla pero no la tabla
...
SELECT * FROM TELEFONO WHERE cast(numero AS INTEGER) BETWEEN 0 AND 5; --Devuelve los entrys donde ese valor numerico este entre esos dos numeros, como en este caso era INT se paso a INTEGER
